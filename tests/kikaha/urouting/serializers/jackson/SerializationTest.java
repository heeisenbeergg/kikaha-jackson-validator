...
public class SerializationTest {
	...
	private final CDI provider = new DefaultCDI();
	private final User user = new User( "gerolasdiwn", new Address("Madison Avenue", 10));
	...
	@Mock
	private BlockingHttpExchange blockingExchange;
	...
	private Void ensureThatWasCorrectlySerialized(InvocationOnMock invocation) throws Throwable {
	...
	public void grantThatUnserializeJSONIntoObjectAsExpected() {
		...
		this.assertIsValidUser(user);
	}

	private void assertIsValidUser(final User user) {
		assertNotNull(user);
	...
	class JSONContentTypeCondition<T> implements Condition<T> {
		...
		@Override
		public boolean check(final T arg0) {
